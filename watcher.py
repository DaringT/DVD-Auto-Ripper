...
def on_devices_changed(drives: List[Drive]):
	compact_discs = [d for d in drives if d.is_compact_disc]
	# compact_discs = [d for d in drives if d.is_removable]
	logger.debug(f'Connected removable drives: {compact_discs}')
	for drive in compact_discs:
